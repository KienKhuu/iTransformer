...
class Model(nn.Module):
    ...
    def encode(self, x_enc, x_mark_enc=None, mask=None):
        """Return the representation after all mixer blocks, before forecasting."""
        for i in range(self.layer):
            x_enc = self.model[i](x_enc)
        # Preserve both temporal and channel information for the CL projection head.
        return x_enc.reshape(x_enc.size(0), -1)

    def forecast(self, x_enc, x_mark_enc, mask=None):
